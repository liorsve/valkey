...
/* 22. Same-size replacement must not corrupt tracked_size.
 * When lpInsert sees new_listpack_bytes == old_listpack_bytes it may skip
 * lp_realloc.  On the min-overhead branch this could leave lp_last_alloc_size
 * stale; on the zmalloc_size branch it should always be correct.  Either way,
 * this test ensures tracked_size stays accurate after same-size replacements
